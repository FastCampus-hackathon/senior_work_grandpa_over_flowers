...
const SearchInput = ({ data }) => {
  ...
  // 검색어 저장
  const [searchSigun, setSearchsearchSigun] = useState("");
  // 지역명 저장
  ...
  const searchEvent = (e) => {
    ...
    const newFilter = data.filter((value) => {
      return value.SIGUN_NM._text.includes(searchSigun);
    });
  ...
  const handleFilter = (event) => {
    setSearchsearchSigun(event.target.value);
  };
  ...
  //   navigate(`/jobResult/${id}`, { replace: true, state: data });
  // }, [navigate, id, data]);

  useEffect(() => {
    const grouped = groupBy(data, "SIGUN_NM");
    setSigun(Object.keys(grouped));
  }, [data]);
  ...
  return (
    <StyledSearchInput>
      ...
      {searchSigun.length !== 0 && (
        <div className="search-result">
          {sigun
            .filter((value) => {
              if (searchSigun === "") {
                return value;
              } else if (value.includes(searchSigun)) {
                return value;
              }
            })
            .map((sigun, index) => {
